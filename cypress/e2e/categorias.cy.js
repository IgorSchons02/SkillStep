describe('Gestão de Categorias - SkillStep', () => {
  ...
  it('Deve criar uma nova categoria com cor personalizada', () => {
    ...
    cy.get('#modalNovaCategoria').within(() => {
      ...
      cy.get('textarea[name="descricao"]')
        .clear()
        .type('Treinamentos focados em segurança da informação.', { delay: 50 });
  ...
  it('Deve editar uma categoria existente através do dropdown', () => {
    // 1. Abre o dropdown do primeiro card
    cy.get('.category-card').first().find('button[data-bs-toggle="dropdown"]').click();
  ...
  it('Deve testar a busca real-time com debounce', () => {
    ...
    cy.get('.row.g-4').then(($el) => {
      if ($el.find('.category-card').length > 0) {
        ...
        cy.contains('Nenhuma categoria encontrada').should('be.visible');
      }
    });
  });

  it('Deve cancelar a exclusão de uma categoria no SweetAlert2', () => {
    cy.get('.category-card').first().find('button[data-bs-toggle="dropdown"]').click();
