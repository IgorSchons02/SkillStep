describe("Gestão de Usuários - SkillStep", () => {
    ...
    it("Deve cadastrar um novo usuário com sucesso via modal", () => {
        ...
        cy.get('#modalNovoUsuario input[name="cpf"]').type("49891923037");

        cy.get('#modalNovoUsuario select[name="tipo_usuario"]').select(
            "Administrador",
        );
        cy.get('#modalNovoUsuario input[name="email"]').type(
            "igor.teste@cigam.com.br",
        );
